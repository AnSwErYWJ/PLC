...
void Get_data(void)
{
   ...
   //# single_sensor(room_info[0],4); //room_id is 4
    ...
    //1Send_data(1); //here used to send room1 data
    sleep(3);
...
void *Control_controller(void *arg)
{
    ...
    while(1)
    {
        ...
        //3Cs = controller_state__unpack(NULL,ret,buffer);
        
        printf("id:");
        scanf("%d",&Cs->ctrl_id);
        printf("state:");
        scanf("%d",&Cs->ctrl_state);
